...
async def fetch_full_scan_data(symbols: list[str], nifty_symbol: str) -> dict:
    """One concurrent batch: Nifty index + all symbols' 15m and 1h candles."""
    async with httpx.AsyncClient() as client:
        nifty_task = _fetch_one(client, nifty_symbol, "15m", "5d")
        s15_tasks = [_fetch_one(client, s, "15m", "5d") for s in symbols]
        s1h_tasks = [_fetch_one(client, s, "1h", "1mo") for s in symbols]
        results = await asyncio.gather(nifty_task, *s15_tasks, *s1h_tasks)

    nifty_raw = results[0]
    s15_results = results[1:1 + len(symbols)]
    s1h_results = results[1 + len(symbols):]

    return {
        "nifty": nifty_raw,
        "15m": {r["symbol"]: r for r in s15_results},
        "1h": {r["symbol"]: r for r in s1h_results},
    }
