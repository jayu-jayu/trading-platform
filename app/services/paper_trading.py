...
async def open_candidate_position(db: AsyncSession, candidate: dict) -> Position | None:
    """
    For DEVELOPING-tier cards, which aren't auto-persisted by the scanner
    (see services/scanner.py). Persists the candidate as a SignalHistory
    row at the moment someone chooses to act on it — so there's still a
    real audit trail, just created lazily instead of eagerly — then opens
    a position against it exactly like open_position() does.
    """
    from app.models.signal import SignalHistory
    ...
    record = SignalHistory(
        ...
        vwap_value=candidate.get("vwap_value"),
        volume_ratio=candidate.get("volume_ratio"),
        rules_passed=candidate.get("rules_passed", []),
        sector_proxy=candidate.get("sector_proxy"),
        market_regime=candidate["market_regime"],
        strength_score=candidate.get("strength_score"),
        tier=candidate.get("tier", "DEVELOPING"),
    )
    db.add(record)
    await db.flush()  # assigns record.id without a full commit yet

    return await open_position(db, record)
