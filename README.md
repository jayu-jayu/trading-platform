## ⚠️ Required step before your NEXT deploy: one manual DB change

This update adds a `tier` column to `signal_history`. `Base.metadata.create_all`
(what this app uses instead of Alembic) only creates **missing tables** — it
does **not** add columns to a table that already exists. Since you already
deployed and Render already created `signal_history` without this column,
you need to add it manually, once, before deploying this update. Two options:

**Option A — one SQL command (keeps your existing paper trading history):**
Open your Render Postgres instance's connection shell (Render dashboard →
your database → **Connect** → **PSQL Command**) and run:
```sql
ALTER TABLE signal_history ADD COLUMN tier VARCHAR(20) DEFAULT 'INSTITUTIONAL';
```

**Option B — wipe and let it recreate (loses existing history, keeps nothing at risk since it's paper trading):**
In the same PSQL shell:
```sql
DROP TABLE IF EXISTS positions, signal_history, portfolio CASCADE;
```
The app recreates all three automatically on next startup.

Skip this and the app will crash on startup trying to persist a signal with
a `tier` field the database doesn't have a column for.
